-- Migration 008: Equipped title for cosmetic display
-- Run AFTER 007_banner_color.sql. Safe to re-run.
--
-- Stores the human-readable title (e.g., "Daily Hunter") so it can be
-- shown under the @username on leaderboards and the profile viewer.
-- Capped at 30 chars; nullable.

ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS equipped_title text
    CHECK (equipped_title IS NULL OR length(equipped_title) <= 30);
