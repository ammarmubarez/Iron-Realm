...
const emgUrl = dataUrl(readFileSync(join(here, "../../src/data/emg.js"), "utf8"));
const load = async (f) => import(dataUrl(readFileSync(join(here, "../../src/data", f), "utf8").replace('from "./emg"', `from "${emgUrl}"`)));
...
const credit=a=>Math.pow(Math.max(0,Math.min(100,a))/100,1.5);
function shares(ex,muscle){ const emg=emgFor(ex); const best={}; for(const [id,a] of Object.entries(emg)){ const st=SVG_TO_STAT[id]||muscle; best[st]=Math.max(best[st]||0,a);} return Object.fromEntries(Object.entries(best).map(([k,a])=>[k,credit(a)])); }
...
console.log("\n== E. Level timeline at 12 hard sets/wk to a muscle (full credit, 2 RIR → 110 XP/wk) ==");
let cum=0; const out=[]; for(let l=1;l<=40;l++){ cum+=MUSCLE_THRESHOLDS[l-1]; if([2,3,4,5,7,10,12,20,30,40].includes(l+1)) out.push(`L${l+1}: ${cum} XP ≈ ${(cum/110).toFixed(0)} wk (${(cum/110/52).toFixed(1)} y)`);} console.log("  "+out.join("\n  "));
